---
id: merged-your-upkeep-scope-02
status: new
card: multiple
created: 2026-04-15T04:57:48Z
kind: consolidated
source_tickets: angel_of_flight_alabaster-01, bloodgift_demon-02, mayor_of_avabruck-03, splinterfright-01, cloistered_youth-02, merged-your-upkeep-scope-01
---

# "At the beginning of your upkeep" fires during all players' upkeeps (CR 603.2)

## Description
Oracle text "at the beginning of your upkeep" (and analogously "your end step") constrains the trigger event to the controller's step. Per CR 603.2, the trigger fires only when its trigger event occurs. The engine's step-started dispatch (triggers.rs:815-862) queues an UpkeepTrigger / EndStepTrigger for every permanent with a matching TriggerKind, regardless of whose step it is. Card handlers compensate by early-returning if `state.active_player != controller`, but the trigger is still created, placed on the stack, and observable. The TriggerKind enum has no "your" vs "each" scope.

## Engine path
- triggers.rs:815-862 (upkeep/end-step trigger dispatch — no controller filter)
...
### test_cloistered_youth_no_phantom_upkeep_trigger
Source ticket: cloistered_youth-02
Implementation: (not yet written)
...
### test_unholy_fiend_no_phantom_end_step_trigger
Source ticket: cloistered_youth-02
Implementation: (not yet written)
Scenario: Player A controls Unholy Fiend (transformed Cloistered Youth). During Player B's end step, verify that NO end step trigger for Unholy Fiend is placed on the stack, and Player A does NOT lose 1 life.

## Also closes

- merged-your-upkeep-scope-01
