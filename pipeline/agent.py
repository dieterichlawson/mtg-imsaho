...
def run_agent_in(prompt: str, cwd: Path,
    ...
    if paths.AGENT_SETTINGS.exists():
        cmd += ["--settings", str(paths.AGENT_SETTINGS)]
    ...
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                            text=True, cwd=str(cwd), env=_subscription_env(),
                            bufsize=1)
    tokens = tool_uses = 0
    is_error = False
...
def run_agent(prompt: str, model: str = DEFAULT_MODEL,
    ...
    return run_agent_in(prompt, paths.PROJECT_ROOT, model, effort,
...
def run_agent_loop(
    *, build_prompt: Callable[[str, int], str], cwd: Path,
    ...
    model: str = DEFAULT_MODEL, effort: str = DEFAULT_EFFORT,
    log_prefix: str = "", progress_prefix: str = "",
) -> tuple[Any, dict]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        log_path = paths.LOGS_DIR / f"{log_prefix}-attempt{attempt}.log"
        result = run_agent_in(prompt, cwd, model, effort,
                              log_path=log_path,
...
def _load_or_describe(result: dict, staging_file: Path,
                      loader: Callable[[Path], Any]) -> tuple[Any, str | None]:
    ...
    if not staging_file.exists():
        return None, (f"Previous attempt did not write {staging_file.name}. "
    ...
    try:
        ...
        parsed = None
        error = (f"Your staging JSON failed validation: {e}\n"
...
def build_prompt(role: str, **ctx) -> Callable[[str, int], str]:
    ...
    """
    shared = (paths.PROMPTS_DIR / f"{role}.md").read_text()
    per_path = paths.PROMPTS_DIR / f"{_PER_AGENT_NAME[role]}.peragent.md"
