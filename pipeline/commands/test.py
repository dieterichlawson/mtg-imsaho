...
from pipeline import oracle, ticket, validate, worktree
...
from pipeline.staging import TEST_BLOCKED, TEST_CONFIRMED, TEST_REJECTED, load_test
from pipeline.state import Status, TestOutcome, after_test
...
_FIELD_PREAMBLE = (
    "Your staging JSON is missing required fields:\n")
_FIELD_CODA = (
...
@dataclass
class TestResult:
    """The per-test agent output + Python's verdict on each."""
...
def _prepare_staging(wt: Path, ticket_id: str) -> Path:
    ...
    return wt / "pipeline" / "staging" / f"{ticket_id}-test.json"


def _run_test_writer(t: Ticket, wt: Path, staging_file: Path, args
                     ) -> tuple[TestResult, dict]:
    oracle_text = oracle.get_oracle_text(t.card) or "Oracle text not available"
    ...
    outcome = TestResult(test_file=default_file)

    builder = build_prompt(
        "test-writer", ticket_body=t.body, oracle=oracle_text,
        engine_note=engine_note, tid_snake=tid_snake, tid=t.id)

    def validator(parsed, _result, attempt):
        return _validate_test_run(parsed, attempt, wt, default_file,
                                  expected_slugs, outcome)

    print(f"  [{t.id}] Spawning agent in worktree...")
    _, result = run_agent_loop(
        build_prompt=builder, cwd=wt, staging_file=staging_file,
        loader=load_test, validator=validator,
...
def _validate_test_run(parsed: dict, attempt: int, wt: Path,
                       default_file: str, expected_slugs: set[str],
                       out: TestResult) -> str | None:
    ...
    if missing:
        ...
        if attempt < MAX_ATTEMPTS:
            return (f"Ticket's `## Tests` section lists {len(expected_slugs)} "
...
def _check_field_completeness(per_test: list[dict]) -> list[str]:
    errors = []
    for t in per_test:
...
def _validate_each_test(per_test: list[dict], wt: Path,
    ...
    for t in per_test:
        ...
        if status == TEST_CONFIRMED:
            ...
            if result.is_confirmed_bug:
                impls[slug] = f"{test_file}::{t['test_name']}"
            else:
                t["status"] = TEST_REJECTED
                impls[slug] = f"rejected: {result.reason}"
        elif status == TEST_REJECTED:
            impls[slug] = f"rejected: {t.get('explanation','')[:80]}"
        elif status == TEST_BLOCKED:
            impls[slug] = "(not yet written)"
    return impls


def _aggregate(per_test: list[dict]) -> TestOutcome:
...
def _apply_test_outcome(t: Ticket, out: TestResult, result: dict, args) -> None:
...
def _apply_status(t: Ticket, out: TestResult, result: dict,
                  model: str, run_id: str) -> None:
    new_status = after_test(Status.NEW, out.aggregate)
    confirmed = sum(1 for e in out.per_test if e["status"] == TEST_CONFIRMED)
    fields = {
        "test_run_id": run_id, "test_model": model,
    ...
    if out.aggregate is TestOutcome.CONFIRMED:
        ...
        fields.update({"allow_engine_edits": "true",
                       "engine_block_at": now_iso()})
    else:
        fields["false_positive_at"] = now_iso()

    t.status = new_status
    t.frontmatter.update(fields)
    t.save()

    if out.aggregate is TestOutcome.NEEDS_ENGINE:
        blockers = [f"- **{e['slug']}**: {e['blocked_by']}"
                    for e in out.per_test
                    if e["status"] == TEST_BLOCKED and e.get("blocked_by")]
        if blockers:
            t.append_section(
...
def _render_results_section(per_test: list[dict]) -> str:
...
def _log(t: Ticket, out: TestResult, result: dict,
         model: str, run_id: str) -> None:
    ...
    log_run("test-writer", run_id=run_id, model=model, card=t.card,
            result=result, finding_id=t.id,
            test_result=out.aggregate.value,
            validation_passed=validated,
            rejection_reason=None if validated
                             else "one-or-more tests failed validation",
            notes=f"{confirmed}/{total} tests confirmed")
    log_finding(t.id, f"test_{out.aggregate.value}", card=t.card,
                run_id=run_id, test_file=out.test_file)


def _summarize(t: Ticket, out: TestResult, result: dict) -> dict:
    confirmed = sum(1 for x in out.per_test if x["status"] == TEST_CONFIRMED)
    total = len(out.per_test)
    print(f"  [{t.id}] Done: {out.aggregate.value} — {confirmed}/{total} "
          f"tests ({result['duration']}s, {result['tokens']} tok)")
    return {"ticket": t.id, "result": out.aggregate.value,
