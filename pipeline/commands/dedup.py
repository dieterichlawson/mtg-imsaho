...
from pipeline.metrics import today
from pipeline.staging import StagingError, load_consolidation
...
def cmd_dedup(args):
    ...
    builder = build_prompt("dedup", num_tickets=len(ids),
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        result = run_agent(
            builder(retry_note, attempt), args.model, args.effort,
            log_path=paths.LOGS_DIR / f"{today()}-dedup-attempt{attempt}.log",
        ...
        new_files = [f for f in _existing_consolidations()
...
def _existing_consolidations() -> list[Path]:
    return sorted(paths.STAGING_DIR.glob("consolidation-*.json"))
...
def _retry_note_parse_errors(attempt: int,
