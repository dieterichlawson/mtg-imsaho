...
from pipeline.state import Status, after_retry
...
def cmd_retry(args):
    ...
    try:
        t.status = after_retry(previous, target, force=args.force)
    ...
    if target is Status.TESTED and not sha:
        print(f"ERROR: {t.id} has no tested_sha; cannot rewind to "
              f"{Status.TESTED.value}. Use --to {Status.NEW.value} instead.")
        sys.exit(1)

    print(f"[{t.id}] Retry: {previous.value} → {target.value}  (reset to {sha})")
    ...
    print(f"[{t.id}] Ready. Run `./pipeline/cli.py {cmd} {flag} {t.id}` to continue.")
