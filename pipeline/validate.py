...
FIX_BANNED_PATTERN = re.compile(
    r"\bTODO|\bFIXME|\bhack|\bworkaround|\btemporary", re.IGNORECASE
)
...
def validate_test(
    ...
    return TestValidation(
        False, "test fails but unclear failure type", output=output[-1500:]
    )


def _has_assertion_failure(output: str) -> bool:
    return bool(
        re.search(r"assertion.*failed|assert.*failed|left.*right", output)
    )
...
def validate_fix(worktree: Path) -> FixValidation:
    ...
    check = subprocess.run(
        ["cargo", "check"], capture_output=True, text=True, cwd=str(worktree)
    )
    check_output = (check.stdout or "") + (check.stderr or "")
    if "warning[" in check_output:
        return FixValidation(
            False, "compiler warnings present", output=check_output[-2000:]
        )

    # 3. Full test suite — exit 0 AND no "FAILED" lines.
    tests = subprocess.run(
        ["cargo", "test"], capture_output=True, text=True, cwd=str(worktree)
    )
