...
def print_funnel(findings: list[dict], queue_counts: dict):
    ...
    if tested > 0:
        print(
            f"    Confirmed (fails):  {confirmed}  ({confirmed * 100 // tested}%)"
        )
        print(
            f"    Rejected (FP):      {rejected}  ({rejected * 100 // tested}%)"
        )
        print(
            f"    Blocked:            {blocked}  ({blocked * 100 // tested}%)"
        )
    print(f"  Fixes attempted:      {fixed}")
    if fixed > 0:
        print(f"    Succeeded:          {fix_ok}  ({fix_ok * 100 // fixed}%)")
        print(
            f"    Failed:             {fix_fail}  ({fix_fail * 100 // fixed}%)"
        )
...
def print_agent_perf(runs: list[dict]):
    ...
    # Print table
    print(
        f"  {'Model':<25} {'Role':<14} {'Runs':>5} {'Validated':>10} {'Key Metric':>20}"
    )
    ...
    for (model, role), runs_list in sorted(by_model_role.items()):
        n = len(runs_list)
        validated = sum(
            1 for r in runs_list if r.get("validation_passed", True)
        )

        # Key metric depends on role
        if role == "auditor":
            total_findings = sum(
                r.get("findings_created", 0) for r in runs_list
            )
            metric = f"{total_findings} findings"
        elif role == "test-writer":
            confirmed = sum(
                1 for r in runs_list if r.get("test_result") == "confirmed"
            )
            rejected = sum(
                1 for r in runs_list if r.get("test_result") == "rejected"
            )
...
def print_trends(findings: list[dict]):
    ...
    print(
        f"  {'Date':<12} {'Created':>8} {'Confirmed':>10} {'Rejected':>9} {'Fixed':>6} {'Merged':>7}"
    )
    ...
    for date in sorted(by_date.keys()):
        c = by_date[date]
        print(
            f"  {date:<12} {c['created']:>8} {c['test_confirmed']:>10} "
...
def print_cards(findings: list[dict]):
    ...
    print(
        f"  {'Card':<30} {'Total':>6} {'Confirmed':>10} {'Rejected':>9} {'Fixed':>6}"
    )
    print(f"  {'-' * 30} {'-' * 6} {'-' * 10} {'-' * 9} {'-' * 6}")

    for card, total, events in card_stats[:30]:
        confirmed = (
            events.get("test_confirmed", 0)
            + events.get("fix_succeeded", 0)
            + events.get("merged", 0)
        )
        rejected = events.get("test_rejected", 0)
...
def print_efficiency(runs: list[dict]):
    ...
    print(
        f"  {'Role':<14} {'Runs':>5} {'Avg Tokens':>11} {'Avg Tools':>10} {'Avg Duration':>13}"
    )
    ...
    for role in ["auditor", "test-writer", "fixer", "log-miner"]:
        ...
        n = len(role_runs)
        tokens = [
            r.get("total_tokens", 0) for r in role_runs if r.get("total_tokens")
        ]
        ...
        print(
            f"  {role:<14} {n:>5} {avg_tok:>11,} {avg_tools:>10} {dur_str:>13}"
        )
    print()
...
def main():
    """Parse args and print the metrics dashboard section(s) requested."""
    parser = argparse.ArgumentParser(description="Pipeline metrics dashboard")
    parser.add_argument(
        "--funnel", action="store_true", help="Show funnel only"
    )
    parser.add_argument(
        "--agents", action="store_true", help="Show agent performance"
    )
    parser.add_argument(
        "--trends", action="store_true", help="Show trends over time"
    )
    parser.add_argument(
        "--cards", action="store_true", help="Show per-card breakdown"
    )
    parser.add_argument(
