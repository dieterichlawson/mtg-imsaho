 ...
 */
export function artNames(name: string): string[] {
  const out: string[] = [];
  const push = (s: string) => { if (s && !out.includes(s)) out.push(s); };
  push(name);
  push(name.replace(/\s*\(#\d+\).*$/, ""));
  for (const n of [...out]) {
    if (n.endsWith(" ability")) push(n.slice(0, -" ability".length));
    const head = n.replace(/\s*\([^)]*\)\s*$/, "");
    const cut = head.lastIndexOf("'s ");
    if (cut > 0) push(head.slice(0, cut));
  }
  return out;
}
...
export function drawArt(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number,
  ...
  ctx.beginPath();
  ctx.moveTo(x, y + h); ctx.lineTo(x + w, y); ctx.lineTo(x + w, y + Math.max(4, h / 3)); ctx.lineTo(x + Math.max(4, w / 3), y + h);
