...
function permBadges(p: PermanentView, state: State): { t: string; c: string }[] {
  ...
  if (p.damage_marked) b.push({ t: `${p.damage_marked}d`, c: "#e05050" });
...
function drawRow(ctx: Ctx, hits: Hit[], state: LiveState, all: PermanentView[][], y: number): void {
  ...
  items.forEach((item, i) => {
    if (hoveredKey && item.some(q => `o${q.object_id}` === hoveredKey)) { hovered = i; return; }
    draw(item, i);
  });
...
function drawStrip(ctx: Ctx, hits: Hit[], state: LiveState, y: number, pid: PlayerId, life: number, handSize: number,
  ...
  // else on the board.
  text(ctx, clip(ctx, `${isYou ? "You" : "Opponent"} (p${pid})${active ? " ★" : ""}${priority ? " ●" : ""}`, 98),
    50, y + 3, { color: active ? "#ffe080" : "#c0b8c8" });
 ...
 */
export function bandLogLines(state: LiveState): string[] {
  const log = state.view.display_log;
  const fresh = log.slice(state.logSince ?? 0);
  if (fresh.length === 0) return log.slice(-2);
 ...
 */
export function outcomeHeadline(state: LiveState, summary: string): string | null {
  const won = /^Game over!\s+p(\d+)\b/.exec(summary);
  if (won) return Number(won[1]) === state.view.you ? "YOU WIN" : "OPPONENT WINS";
  if (/^Game over! It's a draw!/.test(summary)) return "A DRAW";
  return null;
}
...
export function inspectorFacts(state: LiveState, e: IndexEntry): string[] {
  ...
  else if (o.is_copy) out.push("Copy");
...
function promptArea(ctx: Ctx, hits: Hit[], state: LiveState, x: number, y: number, w: number, h: number): void {
  ...
  if (paged) {
    text(ctx, clip(ctx, `${scroll + 1}–${Math.min(rows.length, scroll + perPage)} of ${rows.length} (scroll)`, w - 8, "7px Silkscreen"),
      x + 4, ry + 1, { font: "7px Silkscreen", color: "#8a8090" });
  }
}

/** `scroll` for a panel row list, clamped so the last page is reachable. */
export function clampRowScroll(total: number, perPage: number, scroll: number): number {
 ...
 */
export function modalRows(ui: Ui): { label: string; run?: () => void; pos?: number; cardName?: string | null }[] {
  const rows = ui.rows ?? (ui.order ? ui.order.map((o, pos) => ({ label: o.label, pos })) : []);
  if (ui.mode !== "list" || !ui.query) return rows;
  const q = ui.query.toLowerCase();
  return rows.filter(r => r.label.toLowerCase().includes(q));
}

/** `ui.scroll`, clamped to what `modalRows` can actually show. */
export function clampScroll(ui: Ui, scroll: number): number {
