...
function drawPerm(ctx, hits, state, p, x, y, group) {
    ...
    if (kw)
        text(ctx, kw, x + 3, y + CARD.h - 10, { color: "#c0c8d8", font: "7px Silkscreen" });
...
function drawHandCard(ctx, hits, state, card, x, y, raised) {
    ...
    if (hasVerbs) {
        ...
        ctx.fillRect(x + HAND.w - 8, y + 2, 5, 5);
    }
...
function promptArea(ctx, hits, state, x, y, w, h) {
    ...
    let ty = y + 4;
    if (state.gameOver) {
        for (const l of wrap(ctx, state.gameOver, w - 8, "8px PressStart").slice(0, 6)) {
            text(ctx, l, x + 4, ty, { font: "8px PressStart", color: "#ffe080" });
            ty += 10;
        }
        return;
    }
