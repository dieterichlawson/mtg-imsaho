    ...
    fn choose_cast_targets(&mut self, view: &GameView, spell: &mtg_engine::actions::CastableSpell, legal_actions: &[Action]) -> Action {
        ...
        match &spell.target_spec {
            CastTargetSpec::NoTargets => {
                Action::CastSpell { object_id: spell.object_id, targets: vec![], sacrifice: None, exile_count: None, exile_ids: vec![], alternative_cost: None }
            }
            ...
            CastTargetSpec::UpToTargets { max, options } => {
                ...
                if chosen.is_empty() {
                    ...
                    Action::CastSpell { object_id: spell.object_id, targets: vec![options[0].clone()], sacrifice: None, exile_count: None, exile_ids: vec![], alternative_cost: None }
                } else {
                    Action::CastSpell { object_id: spell.object_id, targets: chosen, sacrifice: None, exile_count: None, exile_ids: vec![], alternative_cost: None }
                }
            }
        }
    }
