...
### Family: Subtype filter family — instance vs registry mismatch

Root cause is Bug BD (`setup_game` doesn't initialize `obj.subtypes` from registry data). This forces every subtype check to juggle two sources, and several call sites get it wrong by reading instance subtypes only or registry subtypes only. The fix is in Bug BD; this family enumerates the call sites that need updating.
...
### Family: Damage helper bypass — non-combat damage skips central pipeline

Bug 9F-002 is the umbrella meta-bug enumerating ten ISD damage cards that write `obj.damage_marked += N` directly instead of going through a central damage helper. This means protection checks, planeswalker loyalty decrement, and replacement effects are silently skipped. The narrower bugs in this family are individual call sites or related symptoms.
...
### Family: Hexproof, protection, and target-filter gaps

Multiple call sites generate target lists without consulting the engine's hexproof / protection helpers, letting illegal targets reach the player choice prompt. The four-line fix is to migrate these call sites to a shared filter helper.
...
### Family: Token copy — `create_token_*` doesn't preserve all source state

The token-copy helpers in `mtg-engine/src/state.rs` lose information when creating tokens that copy a source permanent: dynamic P/T, `is_legendary`, post-creation mutations under Parallel Lives doubling, and the `card_id` propagation across all created copies. Fixing the underlying helper resolves the whole family.
...
### Family: Snapshot anthems — until-end-of-turn effects baked at resolution

Bug AP is the canonical example: 'Creatures get +N/+N until end of turn' is implemented by walking the battlefield at resolution time and pushing per-target ModifyPT effects, instead of installing a single global continuous effect. Creatures entering after the anthem resolves don't get the bonus.
...
### Family: Auto-pick — engine makes choices that should belong to the player

Several cards' cost-payment, search, and reanimation paths auto-pick the first eligible candidate instead of presenting the choice to the player. The shape of the fix mirrors Bug C's resolved sacrifice-cost handling: enumerate one action per (target, sacrifice) combo and let the player pick.
...
**Severity:** low — Traveler's Amulet only, related to Bug 76-003's auto-pick
...
### Family: Transform / DFC bugs

Cards that transform to a back face have several places where the engine reads from the front face when it should consult the back face, or vice versa. Bug 99-002 is the systemic root cause inside `helpers::apply_transform`.
...
- *Bug 99-002 — see "Subtype filter family — instance vs registry mismatch" elsewhere in this file*

---

- *Bug 31-001 — see "Board-state display" elsewhere in this file*

---

- *Bug AO — see "Subtype filter family — instance vs registry mismatch" elsewhere in this file*

---

### Family: X-cost / mana-cost handling

Spells and abilities with `{X}` costs take several shortcuts around the auto-tap pipeline, the action-collapse logic in the LLM player, and the pay-during-resolution path. None of the affected cards currently expose X selection cleanly to the player.
...
- *Bug J — see "Auto-pick — engine makes choices that should belong to the player" elsewhere in this file*

---

- *Bug U — see "Auto-pick — engine makes choices that should belong to the player" elsewhere in this file*

---

### Family: Trigger dispatch and timing

These bugs are about when triggers fire, how they are ordered on the stack, and which zone / face the handler reads when resolving. They do not share a single root cause but they all affect the trigger pipeline.
...
### Family: Counter display and storage

Counters are stored and displayed inconsistently. Some cards abuse `card_state` as a counter store instead of using real counters, and the harness deliberately hides several counter types from the model.
...
### Family: Combat / attack-phase rules

Two combat-rule gaps: forced-attack effects ignoring `can't attack` continuous effects, and non-trample attackers failing to divide damage among multiple blockers per CR 510.1c.
...
- Bug BP — "forced-attack effects ignore 'can't attack' continuous
  ...
  prompts" bugs.
### Family: CDA / effective P/T reads

Characteristic-defining abilities (CR 208.2) work in all zones, but several read paths consult `obj.power` instead of `state.effective_power`, silently returning the base value for Splinterfright / Boneyard Wurm / Sturmgeist / Geist-Honored Monk.
...
- *Bug 17-001 — see "X-cost / mana-cost handling" elsewhere in this file*

---

- *Bug AV — see "Token copy — `create_token_*` doesn't preserve all source state" elsewhere in this file*

---

### Family: Miscellaneous / well-isolated
...
### Family: Identical-name disambiguation

Prompts collapse same-named permanents to a single label so the model can't tell which one it's picking. Bug H1 fixed this for combat prompts; the same shape of fix is needed elsewhere.
...
### Family: Resolution-choice / prompt clarity

Mid-resolution prompts (Yes/No, target, trigger-ordering) are presented without a header describing what's being asked, costing the model real in-game outcomes when it guesses wrong.
...
### Family: Board-state display

The board/hand/graveyard summary hides load-bearing information: certain counter types, transformed DFC names on the stack, effective P/T of CDA creatures, and opponent flashback threats.
...
- *Bug E1-002 — see "CDA / effective P/T reads" elsewhere in this file*

---

### Family: Deck-builder / draft

Harness bugs in the deck-builder retry loop and the pool-validity validator.
...
- *Bug 17-004 — see "Miscellaneous / well-isolated" elsewhere in this file*
