...
1. **Damage source is wrong** (blazing_torch.rs:129): The damage source should be Blazing Torch (the equipment), but the code uses `object_id` (the creature) as the source. Per Scryfall ruling: "The source of the damage is Blazing Torch, not the equipped creature." This matters for protection from creatures vs. protection from artifacts.
2. **Premature log message** (blazing_torch.rs:68): The log on line 68 about "Burning Vengeance deals 2 damage to opponent" references the wrong card -- this is a copy-paste error in a log line.

Note: The `damaged_by` on line 129 also pushes the wrong source (creature instead of torch).
