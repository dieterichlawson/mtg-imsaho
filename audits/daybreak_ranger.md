...
**Colour source**: external, fetched this session — a web search over the Scryfall and mtg.wtf results returned that "Nightfall Predator has a color indicator showing it is green". Not from memory.
**Status**: ISSUE (fixed)
...
- Fixed: `color_indicator: vec![Color::Green]` on the back face (Nightfall Predator is green).
