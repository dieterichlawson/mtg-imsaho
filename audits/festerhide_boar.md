...
**Scryfall Oracle text**: Trample. Morbid — This creature enters with two +1/+1 counters on it if a creature died this turn.
**Scryfall type line**: Creature — Boar
**Status**: ISSUE

1. **Oracle text mismatch (cosmetic/semantic)**: Code oracle says "When Festerhide Boar enters the battlefield, if a creature died this turn, put two +1/+1 counters on Festerhide Boar." but Scryfall says "This creature enters with two +1/+1 counters on it if a creature died this turn." The Scryfall wording is a replacement effect (enters WITH counters), not an ETB trigger. The code uses `TriggerKind::EntersBattlefield` and `on_enter_battlefield`. Functionally similar but technically a different mechanic. File: `mtg-engine/src/cards/festerhide_boar.rs`, lines 24-34.
