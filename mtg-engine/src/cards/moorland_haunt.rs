...
impl CardBehavior for MoorlandHaunt {
    ...
    fn on_activate_ability(&self, state: &mut GameState, object_id: ObjectId, _ability_index: usize, _targets: &[Target], _registry: &CardRegistry) {
        ...
        if creatures_in_gy.len() == 1 {
            // Only one creature — auto-exile and create token.
            ...
            state.create_token_with_subtypes(
                ...
                vec![Keyword::Flying], vec!["Spirit".into()],
            );
            state.log(crate::state::LogLevel::Event,
                "Moorland Haunt created a 1/1 white Spirit token with flying".into());
        } else if creatures_in_gy.len() > 1 {
            // Multiple creatures — let the player choose which to exile.
            let options: Vec<crate::actions::Target> = creatures_in_gy.iter()
            ...
            state.awaiting_action = Some(AwaitingAction::ResolutionChoice {
                ...
                choice: ResolutionChoiceKind::ChooseTarget {
                    description: "Moorland Haunt: choose a creature card to exile from graveyard".into(),
                    options,
                    optional: false,
