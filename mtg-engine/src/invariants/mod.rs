...
pub fn check_settled(state: &GameState, registry: &CardRegistry) -> Vec<String> {
    ...
    //
    // Nothing is exempt: a permanent whose enters-as-a-copy choice is still
    // outstanding has not entered the battlefield at all (CR 614.12b), so
    // there is no window here to carve one for.
    for obj in state.objects_in_id_order() {
