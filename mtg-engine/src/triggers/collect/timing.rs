...
pub(super) fn step_started(
    ...
    if *step == crate::types::Step::EndCombat {
        ...
        for entry in pending {
            c.emit(entry.source_id, entry.source_card_id, entry.controller, entry.description,
                TriggerEvent::DelayedTokenExile { target_id: entry.target_id });
        }
    }
            ...
            if let Some(behavior) = registry.get(card_id) {
                ...
                if !desc.is_empty() {
                    ...
                    let event = match kind {
                        crate::cards::TriggerKind::Upkeep => TriggerEvent::Upkeep,
                        crate::cards::TriggerKind::EndCombat => TriggerEvent::EndCombat,
            ...
            if let Some(behavior) = registry.get(watcher_card_id) {
                ...
                if !desc.is_empty() {
                    c.emit(watcher_id, watcher_card_id, watcher_controller, desc,
                        TriggerEvent::SpellCast { caster: *caster, spell_id: *spell_id });
                }
            }
        }
    }
