...
pub struct GameState {
    ...
    /// Detailed log — every action and event (for file logging).
    pub detailed_log: Vec<String>,

    /// Display log — only significant events (for UI).
...
impl GameState {
    ...
    /// Log to the detailed log only (every action, pass, etc.).
    pub fn log_detail(&mut self, msg: String) {
        self.detailed_log.push(msg);
    }

    /// Log to both detailed and display logs (significant events).
    pub fn log_display(&mut self, msg: String) {
        self.detailed_log.push(msg.clone());
        self.display_log.push(msg);
    }
