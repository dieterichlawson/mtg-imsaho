...
fn creature_spell_goes_on_stack() {
    ...
    let new_state = engine::submit_action(
        &state, &Action::CastSpell { object_id: creature, targets: vec![], sacrifice: None }, &registry,
    );
...
fn full_cast_and_resolve_sequence() {
    ...
    state = engine::submit_action(
        &state, &Action::CastSpell { object_id: tusker, targets: vec![], sacrifice: None }, &registry,
    );
