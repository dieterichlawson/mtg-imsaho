...
/// A collected trigger carries the description of the ability that
/// triggered, picked out of the card's list by kind.
///
/// Fiend Hunter declares two — an enters trigger and a leaves trigger —
/// and a lookup that matched on anything but the kind would hand the
/// player the other one's text.
#[test]
fn a_collected_trigger_carries_its_own_abilitys_text() {
    let reg = registry();
    let mut state = game_at_step(Step::PrecombatMain, P0);
    named_permanent(&mut state, &reg, "Grizzly Bears", P1);
    let card_id = reg.get_id_by_name("Fiend Hunter").unwrap();
    let hunter = state.create_object(card_id, P0, Zone::Hand, None, None);
    state.get_object_mut(hunter).unwrap().name = "Fiend Hunter".into();
    state.events.clear();
    state.trigger_event_index = 0;

    state.move_object(hunter, Zone::Battlefield, &reg);
    // Collect only: `process_triggers` would resolve them and leave the
    // queues empty, and the description is written at collection.
    ...
    let collected: Vec<String> = state.pending_trigger_pushes_ap.iter()
        ...
        .collect();
    assert!(collected.iter().any(|d| d.contains("exile another target creature")),
